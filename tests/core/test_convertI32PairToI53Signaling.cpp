...
double test(int64_t val) {
  ...
  printf("input=%lld / 0x%llx: convertI32PairToI53Signaling(lo=%d hi=%d)=", val, val, lo, hi);
...
#else
  double out = EM_ASM_DOUBLE(try {
    return convertI32PairToI53Signaling($0, $1)
  } catch(e) {
    return NaN;
  } , lo, hi);
#endif
...
int main() {
  printf("Testing library_int53.js function $convertI32PairToI53Signaling():\n\n");
