...
def requires_hardware(hardware):
  """Allow test to be skipped if certain hardware is lacking.

  E.g.:

    @requires_hardware('graphics')
    def test_foobar(self):
      pass

  The test will be skipped if GPU is required but unavailable.

  The environment variable EM_LACKS_HARDWARE_ACCESS can be set to a list
  of missing hardware devices separated by comma, or '1' which means no
  hardware is present.
  """
  all_hardware = [
    'graphics',
  ...
  missing_hardware = os.environ.get('EM_LACKS_HARDWARE_ACCESS')
  if missing_hardware is None:
    missing_hardware = []
  elif missing_hardware == '1':
    missing_hardware = all_hardware
  else:
    missing_hardware = missing_hardware.split(',')
  return unittest.skipIf(hardware in missing_hardware, "This test requires %s hardware access" % hardware)
