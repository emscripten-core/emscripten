...
class sanity(RunnerCore):
  ...
  def test_closure_compiler(self):
    ...
    f.write('CLOSURE_COMPILER = "/tmp/nowhere/nothingtoseehere/kjadsfkjwelkjsdfkqgas/nonexistent.txt"\n')
    f.close()
    output = self.check_working([EMCC, '-O2', '-s', '--closure', '1', path_from_root('tests', 'hello_world.cpp')], CLOSURE_FATAL)

    # With a working path, all is well
    restore()
    try_delete('a.out.js')
    output = self.check_working([EMCC, '-O2', '-s', '--closure', '1', path_from_root('tests', 'hello_world.cpp')], '')
  ...
  def test_llvm_fastcomp(self):
    ...
    self.check_working([EMCC, path_from_root('tests', 'hello_world.cpp'), '-s', 'ASM_JS=0'], '''Compiler settings are incompatible with fastcomp. You can fall back to the older compiler core, although that is not recommended''')
...
''' % (version, NODE_JS))
  ...
  def test_native_optimizer(self):
    ...
    def build():
      return self.check_working([EMCC, '-O2', path_from_root('tests', 'hello_world.c')], 'running js post-opts')
  ...
  def test_embuilder(self):
    ...
    tests = [
      ...
      ([PYTHON, EMBUILDER, 'build', 'zlib'], ['building and verifying zlib', 'success'], True, [os.path.join('ports-builds', 'zlib', 'libz.a')]),
      ([PYTHON, EMBUILDER, 'build', 'libpng'], ['building and verifying libpng', 'success'], True, [os.path.join('ports-builds', 'libpng', 'libpng.bc')]),
      ([PYTHON, EMBUILDER, 'build', 'bullet'], ['building and verifying bullet', 'success'], True, [os.path.join('ports-builds', 'bullet', 'libbullet.bc')]),
      ([PYTHON, EMBUILDER, 'build', 'vorbis'], ['building and verifying vorbis', 'success'], True, [os.path.join('ports-builds', 'vorbis', 'libvorbis.bc')]),
      ([PYTHON, EMBUILDER, 'build', 'ogg'], ['building and verifying ogg', 'success'], True, [os.path.join('ports-builds', 'ogg', 'libogg.bc')]),
      ([PYTHON, EMBUILDER, 'build', 'sdl2'], ['success'], True, [os.path.join('ports-builds', 'sdl2', 'libsdl2.bc')]),
      ([PYTHON, EMBUILDER, 'build', 'sdl2-image'], ['success'], True, [os.path.join('ports-builds', 'sdl2-image', 'libsdl2_image.bc')]),
      ([PYTHON, EMBUILDER, 'build', 'freetype'], ['building and verifying freetype', 'success'], True, [os.path.join('ports-builds', 'freetype', 'libfreetype.a')]),
      ([PYTHON, EMBUILDER, 'build', 'harfbuzz'], ['building and verifying harfbuzz', 'success'], True, [os.path.join('ports-builds', 'harfbuzz', 'libharfbuzz.a')]),
      ([PYTHON, EMBUILDER, 'build', 'sdl2-ttf'], ['building and verifying sdl2-ttf', 'success'], True, [os.path.join('ports-builds', 'sdl2-ttf', 'libsdl2_ttf.bc')]),
      ([PYTHON, EMBUILDER, 'build', 'sdl2-net'], ['building and verifying sdl2-net', 'success'], True, [os.path.join('ports-builds', 'sdl2-net', 'libsdl2_net.bc')]),
      ([PYTHON, EMBUILDER, 'build', 'binaryen'], ['building and verifying binaryen', 'success'], True, []),
      ([PYTHON, EMBUILDER, 'build', 'cocos2d'], ['building and verifying cocos2d', 'success'], True, [os.path.join('ports-builds', 'Cocos2d', 'libCocos2d.bc')]),
  ...
  def test_wacky_env(self):
    ...
    def build():
      return self.check_working([EMCC, path_from_root('tests', 'hello_world.c')], '')
  ...
  def test_vanilla(self):
    ...
    try:
      ...
      self.check_working([EMCC, path_from_root('tests', 'hello_world.c'), '-c'], 'wasm32-unknown-unknown-elf')
      os.environ['EMCC_WASM_BACKEND'] = '0'
      make_fake('asmjs-unknown-emscripten')
      self.check_working([EMCC, path_from_root('tests', 'hello_world.c'), '-c'], 'asmjs-unknown-emscripten')
      del os.environ['EMCC_WASM_BACKEND']
    ...
    def test_with_fake(report, expected):
      ...
      try:
        os.environ['EMCC_DEBUG'] = '1'
        output = self.check_working([EMCC, path_from_root('tests', 'hello_world.c'), '-c'], expected)
    ...
    try:
      os.environ['EMCC_DEBUG'] = '1'
      self.check_working([EMCC, path_from_root('tests', 'hello_world.c'), '-c'], 'use asm.js backend')
      os.environ['LLVM'] = path_from_root('tests', 'fake2', 'bin')
      self.check_working([EMCC, path_from_root('tests', 'hello_world.c'), '-c'], 'regenerating vanilla check since other llvm')
    finally:
      del os.environ['EMCC_DEBUG']
