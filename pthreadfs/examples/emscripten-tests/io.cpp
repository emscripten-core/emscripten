...
// EIO is used by Emscripten, but is also defined (somewhere) as a C macro
#undef EIO 
  EM_PTHREADFS_ASM(
    ...
    var device = await PThreadFS.makedev(major++, 0);
    await PThreadFS.registerDevice(device, {
    ...
    await PThreadFS.mkdev('/device', device);

    var broken_device = await PThreadFS.makedev(major++, 0);
    await PThreadFS.registerDevice(broken_device, {
      read: function(stream, buffer, offset, length, pos) {
        throw new PThreadFS.ErrnoError(ERRNO_CODES.EIO);
      },
      write: function(stream, buffer, offset, length, pos) {
        throw new PThreadFS.ErrnoError(ERRNO_CODES.EIO);
      }
    });
    await PThreadFS.mkdev('/broken-device', broken_device);

    // NB: These are meant to test FS.createDevice specifically,
    //     and as such do not use registerDevice/mkdev
    await PThreadFS.createDevice('/', 'createDevice-read-only', function() {});
    await PThreadFS.createDevice('/', 'createDevice-write-only', null, function() {});

    await PThreadFS.mkdir('/working/folder');
    await PThreadFS.writeFile('/working/file', '1234567890');
  );
#define EIO 5
