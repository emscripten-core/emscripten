...
extern EMSCRIPTEN_RESULT emscripten_set_offscreencanvas_size(const char *target, int width, int height);
...
static void _do_call(void* arg) {
  ...
  switch (q->functionEnum) {
    case EM_PROXIED_RESIZE_OFFSCREENCANVAS:
      q->returnValue.i =
        emscripten_set_offscreencanvas_size(q->args[0].cp, q->args[1].i, q->args[2].i);
      break;
