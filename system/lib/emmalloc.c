...
static void dump_memory_regions() {
  ...
  while (root) {
    ...
    MAIN_THREAD_ASYNC_EM_ASM(out("Region block " + ptrToString($0) + " - " + ptrToString($1) + " ("+toString(Number($2)) + " bytes):"), r, lastRegionEnd, lastRegionEnd - uint8_r);
  ...
  for (int i = 0; i < NUM_FREE_BUCKETS; ++i) {
    ...
    while (fr != &freeRegionBuckets[i]) {
      MAIN_THREAD_ASYNC_EM_ASM(out("In bucket " + $0 + ", free region " + ptrToString($1) + ", size: " + toString(Number($2)) + " (size at ceiling: " + toString(Number($3)) + "), prev: " + ptrToString($4) + ", next: " + ptrToString($5)), i, fr, fr->size, size_of_region_from_ceiling(fr), fr->prev, fr->next);
  ...
  MAIN_THREAD_ASYNC_EM_ASM(out("Free bucket index map: " + toString(Number($0)).toString(2) + " " + toString(Number($1)).toString(2)), 
...
static int validate_memory_regions() {
  ...
  while (root) {
    ...
    if (!debug_region_is_consistent(r)) {
      MAIN_THREAD_ASYNC_EM_ASM(err("Used region " + ptrToString($0) + ", size: " + toString(Number($1)) + " (" + ($2? "used": "--FREE--") + ") is corrupt (size markers in the beginning and at the end of the region do not match!)"), r, r->size, region_ceiling_size(r) == r->size);
      return 1;
    }
  ...
  for (int i = 0; i < NUM_FREE_BUCKETS; ++i) {
    ...
    while (fr != &freeRegionBuckets[i]) {
      if (!debug_region_is_consistent(fr) || !region_is_free(fr) || fr->prev != prev || fr->next == fr || fr->prev == fr) {
        MAIN_THREAD_ASYNC_EM_ASM(out("In bucket " + $0 + ", free region " + ptrToString($1) + ", size: " + toString(Number($2)) + " (size at ceiling: " + toString(Number($3)) + "), prev: " + ptrToString($4) + ", next: 0x" + ptrToString($5) + " is corrupt!"), i, fr, fr->size, size_of_region_from_ceiling(fr), fr->prev, fr->next);
        return 1;
      }
      prev = fr;
      fr = fr->next;
...
static size_t validate_alloc_size(size_t size) {
  ...
  assert(validatedSize > size - 1); // 32-bit wraparound should not occur, too large sizes should be stopped before
...
static int attempt_region_resize(Region *region, size_t size) {
  ...
  if (nextRegion->size != sizeAtCeiling) { // Next region is free?
    ...
    if (newNextRegionStartPtr + sizeof(Region) < nextRegionEndPtr + 1) { // <=
      unlink_from_free_list(nextRegion);
    ...
    if (newNextRegionStartPtr - 1 < nextRegionEndPtr) {
      unlink_from_free_list(nextRegion);
    ...
    if (size + sizeof(Region) - 1 < region->size) {
...
struct mallinfo emmalloc_mallinfo() {
  ...
  while (root) {
    ...
    while (uint8_r < lastRegionEnd) {
      ...
      } else info.uordblks += r_size;
      
      // Update approximate watermark data
      info.usmblks = MAX(info.usmblks, (intptr_t)r + r_size);
...
static int trim_dynamic_heap_reservation(size_t pad) {
  ...
  if (!region_is_free(lastActualRegion) || lastActualRegion->size < newRegionSize + 1) {
    return 0;
  }
