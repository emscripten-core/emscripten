...
int __standalone_month_to_secs(int month, int is_leap)
{
...
long long __standalone_year_to_secs(long long year, int *is_leap)
{
	if (year-2ULL <= 136) {
		int y = year;
		int leaps = (y-68)>>2;
...
long long __standalone_tm_to_secs(const struct tm *tm)
{
	int is_leap;
	...
	long long t = __standalone_year_to_secs(year, &is_leap);
	t += __standalone_month_to_secs(month, is_leap);
...
int __standalone_secs_to_tm(long long t, struct tm *tm)
{
...
{
	struct tm new;
	long long t = __standalone_tm_to_secs(tm);
	if (__standalone_secs_to_tm(t, &new) < 0) {
		errno = EOVERFLOW;
		return -1;
	}
	*tm = new;
...
weak time_t mktime(struct tm *tm)
{
	...
	if (__standalone_secs_to_tm(t + new.__tm_gmtoff, &new) < 0) goto error;
...
weak struct tm *__localtime_r(const time_t *restrict t, struct tm *restrict tm)
{
	...
	if (__standalone_secs_to_tm((long long)*t + tm->__tm_gmtoff, tm) < 0) {
		errno = EOVERFLOW;
		return 0;
	}
	return tm;
...
{
	if (__standalone_secs_to_tm(*t, tm) < 0) {
		errno = EOVERFLOW;
		return 0;
	}
	tm->tm_isdst = 0;
...
weak time_t _timegm_js(struct tm* tm) {
  REPORT_UNSUPPORTED(_timegm_js);
  errno = ENOTSUP;
  return -1;
}

weak time_t _mktime_js(struct tm* tm) {
  REPORT_UNSUPPORTED(_mktime_js);
  errno = ENOTSUP;
  return -1;
}
