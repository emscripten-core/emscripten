  ...
  for dirpath, dirnames, filenames in os.walk(rootpathsrc):
    ...
    for name in dirnames:
      ...
      if not should_ignore(fullname):
        ...
        err(f'Skipping directory "{fullname}" from inclusion in the emscripten virtual file system.')
    for name in filenames:
      ...
      if not should_ignore(fullname):
        ...
        err(f'Skipping file "{fullname}" from inclusion in the emscripten virtual file system.')
    dirnames.clear()
...
def to_asm_string(string):
  ...
  def escape(c):
    ...
    return f'\\{oct(c >> 6)}{oct(c >> 3)}{oct(c >> 0)}'
...
def generate_object_file(data_files):
  ...
  for f in embed_files:
    f.c_symbol_name = f'__em_file_data_{to_c_symbol(f.dstpath, used)}'
  ...
  with open(asm_file, 'w', encoding='utf-8') as out:
    ...
    for f in embed_files:
      if DEBUG:
        err(f'embedding {f.srcpath} at {f.dstpath}')
  ...
  for file_ in data_files:
    if not file_.explicit_dst_path:
      ...
      if not abspath.startswith(curr_abspath):
        err(f'Error: Embedding "{path}" which is not contained within the current directory '
            f'"{curr_abspath}".  This is invalid since the current directory becomes the '
            ...
            'explicitly specify the target location.')
        sys.exit(1)
      file_.dstpath = abspath[len(curr_abspath) + 1:]
      if os.path.isabs(path):
        diagnostics.warn(f'Embedding an absolute file/directory name "{path}" to the '
            'virtual filesystem. The file will be made available in the '
            f'relative path "{file_.dstpath}". You can use the `--preload-file srcpath@dstpath` '
            'syntax to explicitly specify the target location the absolute source '
            'path should be directed to.')
  ...
  if options.obj_output:
    ...
    for file_ in data_files:
      ...
      if current_size + fsize <= PRELOAD_DATA_FILE_LIMIT:
        ...
        diagnostics.error(f'cannot package file {file_.srcpath}, which is larger than maximum individual file size limit {PRELOAD_DATA_FILE_LIMIT // (1024 * 1024)} MB.')
        return 1
      else:
        current_size = fsize
    ...
    if len(file_chunks) > 1:
      diagnostics.warn(f'warning: file packager is splitting bundle into {len(file_chunks)} chunks')
...
def generate_preload_js(data_target, data_files, metadata, js_file):
    ...
    if dirname:
      ...
      for i in range(len(parts)):
        ...
        if partial not in partial_dirs:
          code += f"Module['FS_createPath']({json.dumps('/' + '/'.join(parts[:i]))}, {json.dumps(parts[i])}, true, true);\n"
  ...
  if start > 256 * 1024 * 1024:
    diagnostics.warn(f'file packager is creating an asset bundle of {start // (1024 * 1024)} MB. '
  ...
  if options.has_preloaded:
    if not options.lz4:
      ...
      use_data = f'''// Reuse the bytearray from the XHR as the source for file reads.
          for (var file of metadata['files']) {{
            var name = file['filename'];
            var data = byteArray.subarray(file['start'], file['end']);
            {finish_handler}
          }}
          Module['removeRunDependency']('datafile_{js_manipulation.escape_for_js_string(data_target)}');'''
    else:
      # LZ4FS usage
      ...
      os.unlink(temp)
      preload_plugins = "true" if options.use_preload_plugins else "false"
      escaped_target = js_manipulation.escape_for_js_string(data_target)
      use_data = f'''var compressedData = {meta};
            compressedData['data'] = byteArray;
            assert(typeof Module['LZ4'] === 'object', 'LZ4 not present - was your app build with -sLZ4?');
            await Module['LZ4'].loadPackage({{ 'metadata': metadata, 'compressedData': compressedData }}, {preload_plugins});
            Module['removeRunDependency']('datafile_{escaped_target}');'''
    ...
    ret += f'''
      ...
      var PACKAGE_NAME = '{js_manipulation.escape_for_js_string(data_target)}';
      var REMOTE_PACKAGE_BASE = '{js_manipulation.escape_for_js_string(remote_package_name)}';
    ...
    code += f'''
      ...
      Module['addRunDependency']('datafile_{js_manipulation.escape_for_js_string(data_target)}');\n'''
    ...
    if options.use_preload_cache:
      code += f'''
        ...
        try {{
          ...
          await preloadFallback(e){catch_handler};
        }}
      ...
      if (!fetched) {{
        ...
        fetchPromise = fetchRemotePackage(REMOTE_PACKAGE_NAME, REMOTE_PACKAGE_SIZE){catch_handler};
      }}\n'''
  ...
  ret += f'''
    ...
    if (Module['FS_createPath']) {{
      runWithFS(Module){catch_handler};
    }} else {{
  ...
  if options.separate_metadata:
    ...
    metadata_file = os.path.basename(js_file + '.metadata')
    ret += f'''
  ...
  async function runMetaWithFS() {{
    ...
    var metadataUrl = Module['locateFile'] ? Module['locateFile']('{metadata_file}', '') : '{metadata_file}';
    {node_support_code}
  ...
  if (Module['FS_createPath']) {{
    ...
    loadPackage({json.dumps(metadata)});\n'''
