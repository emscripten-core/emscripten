...
string_view strip_quotes(string_view str) {
  string_view s = trim(str);
...
std::string expand_user(string_view path) {
  if (path.starts_with('~')) {
...
std::string expand_vars(string_view input) {
  ...
  if (!config_file.empty() && fs::exists(config_file)) {
    ...
    while (std::getline(in, line)) {
      ...
      if (eq != string_view::npos) {
        string_view key = trim(tline.substr(0, eq));
        if (key == "LLVM_ROOT" || key == "CACHE") {
          string_view raw_val = trim(tline.substr(eq + 1));
          if (raw_val.find('+') != string_view::npos ||
              (!raw_val.empty() && raw_val.front() != '\'' &&
               raw_val.front() != '"' && raw_val.front() != '$')) {
            config.failure = true;
            config.failure_reason =
              "Complex expression in config file for " + std::string(key) + ": " + std::string(raw_val);
            continue;
          }
