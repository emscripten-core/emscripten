...
function run(args, printOutput) {
  ...
  if (sourceMap) {
    assert(source.length == 1); // Must present exactly one input JS file when doing source map -> symbol map translation
    var symbolMap = createSymbolMapFromSourceMap(sourceMap, src);
