...
def calculate(temp_files, in_temp, stdout_, stderr_, forced=[]):
  ...
  def create_libc(libname):
    ...
    blacklist = set(
      ...
      ['memcpy.c', 'memset.c', 'memmove.c', 'getaddrinfo.c', 'getnameinfo.c', 'inet_addr.c', 'res_query.c', 'gai_strerror.c', 'proto.c', 'gethostbyaddr.c', 'gethostbyaddr_r.c', 'gethostbyname.c', 'gethostbyname2_r.c', 'gethostbyname_r.c', 'gethostbyname2.c', 'usleep.c'] + # individual files
  ...
  def create_pthreads(libname):
    ...
    pthreads_files = filter(lambda x: os.path.basename(x) not in ('pthread_setcancelstate.c', 'pthread_getattr_np.c', 'pthread_testcancel.c', 'synccall.c'), pthreads_files)
