...
def get_base_cflags(build_dir, force_object_files=False, preprocess=True):
    ...
    flags += [
        f"-ffile-prefix-map={source_dir}={DETERMINISTIC_PREFIX}",
...
"""
    ...
    ensure_target_in_ninja_file(
        get_top_level_ninja_file(), f"subninja {escape_ninja_path(filename)}"
    )
...
class Library:
    ...
    def build(self):
        ...
        """
        return cache.get(
            self.get_path(), self.do_build, force=USE_NINJA == 2, quiet=USE_NINJA
        )

    def generate(self):
        return cache.get(
            self.get_path(), self.do_generate, force=USE_NINJA == 2, quiet=USE_NINJA
        )
    ...
    def get_files(self):
        ...
        if self.src_dir:
            ...
            if self.src_files:
                ...
                return glob_in_path(
                    self.src_dir, self.src_glob, self.src_glob_exclude or ()
                )
    ...
    def generate_ninja(self, build_dir, libname):
        ...
        create_ninja_file(
            ...
            cflags,
            asflags=asflags,
            customize_build_flags=self.customize_build_cmd,
        )
        ...
        if batch_inputs:
            ...
            for cmd, srcs in batches.items():
                ...
                for i in range(0, len(srcs), chunk_size):
                    ...
                    commands.append(
                        building.get_command_with_possible_response_file(
                            cmd + chunk_srcs
                        )
                    )
    ...
    def get_cflags(self):
        ...
        cflags += get_base_cflags(
            self.build_dir, force_object_files=self.force_object_files
        )

        if self.includes:
            cflags += [
                "-I" + utils.path_from_root(i) for i in self._inherit_list("includes")
            ]
        return cflags
    ...
    def variations(cls):
        ...
        vary_on = cls.vary_on()
        return [
            dict(zip(vary_on, toggles))
            for toggles in itertools.product([False, True], repeat=len(vary_on))
        ]
...
class ExceptionLibrary(Library):
    ...
    def variations(cls):
        combos = super().variations()
        return (
            [dict(eh_mode=Exceptions.NONE, **combo) for combo in combos]
...
class SjLjLibrary(Library):
    ...
    def get_cflags(self):
        ...
        if self.eh_mode == Exceptions.EMSCRIPTEN:
            cflags += ["-sSUPPORT_LONGJMP=emscripten", "-sDISABLE_EXCEPTION_THROWING=0"]
        elif self.eh_mode == Exceptions.WASM_LEGACY:
            cflags += [
...
class llvmlibc(DebugLibrary, AsanInstrumentedLibrary, MTLibrary):
    ...
    def get_files(self):
        ...
        files += glob_in_path(
            "system/lib/llvm-libc/src/string",
            "**/*.cpp",
            excludes=["memset.cpp", "memcpy.cpp"] if self.is_asan else [],
        )
        ...
        files += glob_in_path("system/lib/llvm-libc/src/setjmp", "**/*.cpp")
        files += glob_in_path(
            "system/lib/llvm-libc/src/stdlib",
            "*.cpp",
            excludes=[
                "at_quick_exit.cpp",
                "quick_exit.cpp",
        ...
        files += glob_in_path(
            "system/lib/llvm-libc/src/math/generic",
            "**/*.cpp",
            excludes=["atan2l.cpp", "exp_utils.cpp"],
        )
        files += glob_in_path(
            "system/lib/llvm-libc/src/__support/StringUtil", "**/*.cpp"
        )
        return files


class libc(MuslInternalLibrary, DebugLibrary, AsanInstrumentedLibrary, MTLibrary):
    name = "libc"
    ...
    def get_libcall_files(self):
        ...
        math_files = files_in_path(
            path="system/lib/libc/musl/src/math", filenames=math_files
        )

        exit_files = files_in_path(
            path="system/lib/libc/musl/src/exit", filenames=["atexit.c"]
        )
        ...
        iprintf_files += files_in_path(
            path="system/lib/libc/musl/src/string", filenames=["strlen.c"]
        )
    ...
    def get_files(self):
        ...
        libc_files += files_in_path(
            path="system/lib/libc/musl/src/malloc", filenames=["reallocarray.c"]
        )

        libc_files += files_in_path(
            path="system/lib/libc/musl/src/sched", filenames=["sched_yield.c"]
        )
        ...
        libc_files += files_in_path(
            path="system/lib/pthread", filenames=["thread_profiler.c"]
        )
...
class libc_optz(libc):
    ...
    def get_files(self):
        ...
        # some files also appear in libc, and a #define affects them
        mem_files = files_in_path(
            path="system/lib/libc/musl/src/string", filenames=["memcmp.c"]
        )
    ...
    def can_use(self):
        ...
        return (
            super().can_use()
            and settings.SHRINK_LEVEL >= 2
            and not settings.LINKABLE
            and not os.environ.get("EMCC_FORCE_STDLIBS")
        )
...
class libprintf_long_double(libc):
    ...
    def get_files(self):
        return files_in_path(
            path="system/lib/libc/musl/src/stdio", filenames=["vfprintf.c"]
        )
...
class libwasm_workers(DebugLibrary):
    ...
    def get_files(self):
        ...
        return files_in_path(path="system/lib/wasm_worker", filenames=files)

    def can_use(self):
        # see src/library_wasm_worker.js
        return (
            super().can_use() and not settings.SINGLE_FILE and not settings.RELOCATABLE
        )
...
class libsockets(MuslInternalLibrary, MTLibrary):
    ...
    def get_files(self):
        return files_in_path(
            path="system/lib/libc/musl/src/network", filenames=LIBC_SOCKETS
        )
...
class libsockets_proxy(MTLibrary):
    ...
    def get_files(self):
        return [
            utils.path_from_root("system/lib/websocket/websocket_to_posix_socket.c")
        ]
...
class libcxxabi(ExceptionLibrary, MTLibrary, DebugLibrary):
    ...
    def get_files(self):
        ...
        return files_in_path(path="system/lib/libcxxabi/src", filenames=filenames)
...
class libunwind(ExceptionLibrary, MTLibrary):
    ...
    def can_use(self):
        return super().can_use() and self.eh_mode in (
            Exceptions.WASM_LEGACY,
            Exceptions.WASM,
        )
...
class libmalloc(MTLibrary):
    ...
    def __init__(self, **kwargs):
        ...
        if self.malloc not in (
            ...
            raise Exception(
                'malloc must be one of "emmalloc[-debug|-memvalidate][-verbose]", "mimalloc", "dlmalloc" or "none", see settings.js'
            )
    ...
    def get_files(self):
        malloc_base = (
            self.malloc.replace("-memvalidate", "")
            .replace("-verbose", "")
            .replace("-debug", "")
        )
        malloc = utils.path_from_root(
            "system/lib",
            {
                "dlmalloc": "dlmalloc.c",
                "emmalloc": "emmalloc.c",
    ...
    def variations(cls):
        ...
        return (
            [
                dict(malloc="dlmalloc", **combo)
                for combo in combos
                if not combo["memvalidate"] and not combo["verbose"]
            ]
            + [
                dict(malloc="emmalloc", **combo)
                for combo in combos
                if not combo["memvalidate"] and not combo["verbose"]
            ]
            + [
                dict(malloc="emmalloc-memvalidate-verbose", **combo)
                for combo in combos
                if combo["memvalidate"] and combo["verbose"]
            ]
            + [
                dict(malloc="emmalloc-memvalidate", **combo)
                for combo in combos
                if combo["memvalidate"] and not combo["verbose"]
            ]
            + [
                dict(malloc="emmalloc-verbose", **combo)
                for combo in combos
                if combo["verbose"] and not combo["memvalidate"]
            ]
        )
...
class libwasmfs(DebugLibrary, AsanInstrumentedLibrary, MTLibrary):
    ...
    def get_default_variation(cls, **kwargs):
        return super().get_default_variation(
            ignore_case=settings.CASE_INSENSITIVE_FS, **kwargs
        )
...
class libwasmfs_noderawfs(Library):
    ...
    def get_files(self):
        return files_in_path(
            path="system/lib/wasmfs/backends", filenames=["noderawfs_root.cpp"]
        )
...
class libsanitizer_common_rt(CompilerRTLibrary, MTLibrary):
    ...
    includes = ["system/lib/compiler-rt/lib", "system/lib/libc"]
    never_force = True
    cflags = [
...
class libstandalonewasm(MuslInternalLibrary):
    ...
    def get_default_variation(cls, **kwargs):
        return super().get_default_variation(
            ...
            nocatch=settings.DISABLE_EXCEPTION_CATCHING
            and not settings.WASM_EXCEPTIONS,
            **kwargs,
        )
    ...
    def get_files(self):
        files = files_in_path(
            ...
            filenames=["standalone.c", "standalone_wasm_stdio.c", "__main_void.c"],
        )
        ...
        files += files_in_path(
            path="system/lib/libc/musl/src/exit", filenames=["assert.c", "exit.c"]
        )
        return files
...
def get_libs_to_link(options):
    ...
    if only_forced:
        ...
        diagnostics.warning(
            "deprecated",
            "EMCC_ONLY_FORCED_STDLIBS is deprecated.  Use `-nostdlib` to avoid linking standard libraries",
        )
    if force == "1":
        force_include = [
            name for name, lib in system_libs_map.items() if not lib.never_force
        ]
        ...
        if settings.MALLOC == "mimalloc":
            ...
            if settings.USE_ASAN:
                ...
                utils.exit_with_error(
                    "mimalloc is not compatible with -fsanitize=address"
                )
    ...
    if settings.WASM_WORKERS and (
        not settings.SINGLE_FILE and not settings.RELOCATABLE
    ):
