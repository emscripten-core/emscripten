...
def get(ports, settings, shared):
    if settings.USE_REGAL_GLSLOPT == 1:
        ...
        def create():
            ...
            source_path = os.path.join(ports.get_dir(), 'regal', 'regal-' + TAG, 'src', 'glsl')
            dest_path = os.path.join(ports.get_build_dir(), 'glslopt', 'src', 'glsl')
            ...
            commands = []
            o_s = []
            includes = os.join(dest_path, 'include')
            includesglsl = os.join(dest_path,'src','glsl')
            includesmesa = os.join(dest_path,'src','mesa')
            for src in srcs:
                c = os.path.join(dest_path, src)
            ...
            final = os.path.join(ports.get_build_dir(), 'glslopt', 'libglslopt.bc')
...
def process_args(ports, args, settings, shared):
    if settings.USE_REGAL_GLSLOPT == 1:
        ...
        print "include path:"
        print os.path.join(ports.get_build_dir(), 'glslopt', 'src', 'glsl')
        args += ['-Xclang', '-isystem' + os.path.join(ports.get_build_dir(), 'glslopt', 'src', 'glsl')]
    return args
