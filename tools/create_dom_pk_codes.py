...
for s in input_strings:
  h_file.write(
    '#define '
    + pad_to_length(s[2], longest_dom_pk_code_length())
    + ' 0x%04X /* "%s */' % (s[0], pad_to_length(s[1] + '"', longest_key_code_length() + 1))
    + '\n'
  )
...
for s in input_strings:
  c_file.write(
    '    case 0x%08XU /* %s */: return %s /* 0x%04X */'
    % (
      str_to_hash[s[1]],
      pad_to_length(s[1], longest_key_code_length()),
      pad_to_length(s[2] + ';', longest_dom_pk_code_length() + 1),
      s[0],
    )
    + '\n'
  )
...
for s in input_strings:
  if len(s) == 3:
    c_file.write(
      '    case %s return "%s";' % (pad_to_length(s[2] + ':', longest_dom_pk_code_length() + 1), s[2]) + '\n'
    )
