...
class Building:
  ...
  def get_building_env(native=False):
    ...
    env['CFLAGS'] = env['EMMAKEN_CFLAGS'] = ' '.join(Building.COMPILER_TEST_OPTS) + ' --ignore-dynamic-linking ' + env.get('CFLAGS', '')
    env['CXXFLAGS'] = '--ignore-dynamic-linking ' + env.get('CXXFLAGS', '')
    env['LDFLAGS'] = '--ignore-dynamic-linking ' + env.get('LDFLAGS', '')
