...
INCOMPATIBLE_SETTINGS = [
    ...
    (
        ...
        "NO_GL_SUPPORT_SIMPLE_ENABLE_EXTENSIONS",
        None,
    ),
...
LEGACY_SETTINGS = [
    ...
    [
        ...
        [2],
        "Starting from Emscripten 1.37.29, no longer available (https://github.com/emscripten-core/emscripten/pull/6091)",
    ],
    ...
    [
        "BUILD_AS_SHARED_LIB",
        [0],
        "Starting from Emscripten 1.38.16, no longer available (https://github.com/emscripten-core/emscripten/pull/7433)",
    ],
    [
        "SAFE_SPLIT_MEMORY",
        [0],
        "Starting from Emscripten 1.38.19, SAFE_SPLIT_MEMORY codegen is no longer available (https://github.com/emscripten-core/emscripten/pull/7465)",
    ],
    [
        "SPLIT_MEMORY",
        [0],
        "Starting from Emscripten 1.38.19, SPLIT_MEMORY codegen is no longer available (https://github.com/emscripten-core/emscripten/pull/7465)",
    ],
    [
        "BINARYEN_METHOD",
        ["native-wasm"],
        "Starting from Emscripten 1.38.23, Emscripten now always builds either to Wasm (-sWASM - default), or to JavaScript (-sWASM=0), other methods are not supported (https://github.com/emscripten-core/emscripten/pull/7836)",
    ],
    [
        "BINARYEN_TRAP_MODE",
        [-1],
        "The wasm backend does not support a trap mode (it always clamps, in effect)",
    ],
    [
        "PRECISE_I64_MATH",
        [1, 2],
        "Starting from Emscripten 1.38.26, PRECISE_I64_MATH is always enabled (https://github.com/emscripten-core/emscripten/pull/7935)",
    ],
    [
        "MEMFS_APPEND_TO_TYPED_ARRAYS",
        [1],
        "Starting from Emscripten 1.38.26, MEMFS_APPEND_TO_TYPED_ARRAYS=0 is no longer supported. MEMFS no longer supports using JS arrays for file data (https://github.com/emscripten-core/emscripten/pull/7918)",
    ],
    ["ERROR_ON_MISSING_LIBRARIES", [1], "missing libraries are always an error now"],
    [
        "EMITTING_JS",
        [1],
        "The new STANDALONE_WASM flag replaces this (replace EMITTING_JS=0 with STANDALONE_WASM=1)",
    ],
    [
        "SKIP_STACK_IN_SMALL",
        [0, 1],
        "SKIP_STACK_IN_SMALL is no longer needed as the backend can optimize it directly",
    ],
    ...
    [
        ...
        [0, 1],
        "Duplicate function elimination for wasm is handled automatically by binaryen",
    ],
    [
    ...
    [
        "WASM_OBJECT_FILES",
        [0, 1],
        "For LTO, use -flto or -fto=thin instead; to disable LTO, just do not pass WASM_OBJECT_FILES=1 as 1 is the default anyhow",
    ],
    ...
    [
        ...
        [0],
        "Fully swappable asm modules are no longer supported",
    ],
    ...
    [
        "SEPARATE_ASM",
        [0],
        "Separate asm.js only made sense for fastcomp with asm.js output",
    ],
    [
    ...
    [
        ...
        [0, 1],
        "The wasm backend implements memcpy/memset in C",
    ],
    ...
    [
        ...
        [0, 1],
        "The wasm backend always uses a single index space for function pointers, in a single Table",
    ],
    [
        "AGGRESSIVE_VARIABLE_ELIMINATION",
        [0, 1],
        "Wasm ignores asm.js-specific optimization flags",
    ],
    ...
    [
        "WASM_BACKEND",
        [-1],
        "Only the wasm backend is now supported (note that setting it as -s has never been allowed anyhow)",
    ],
    ["EXPORT_BINDINGS", [0, 1], "No longer needed"],
    [
        "RUNNING_JS_OPTS",
        [0],
        "Fastcomp cared about running JS which could alter asm.js validation, but not upstream",
    ],
    ...
    [
        ...
        [0],
        "Wasm2JS does not support iPhone 4s, iPad 2, iPad 3, iPad Mini 1, Pod Touch 5 (devices with end-of-life at iOS 9.3.5) and older",
    ],
    ...
    [
        "HEADLESS",
        [0],
        "No longer supported, use headless browsers or Node.js with JSDOM",
    ],
    [
        "USE_OFFSET_COVERTER",
        [0],
        "No longer supported, not needed with modern v8 versions",
    ],
    ["ASYNCIFY_LAZY_LOAD_CODE", [0], "No longer supported"],
    [
        "USE_WEBGPU",
        [0],
        "No longer supported; replaced by --use-port=emdawnwebgpu, which implements a newer (but incompatible) version of webgpu.h - see tools/ports/emdawnwebgpu.py",
    ],
...
class SettingsManager:
    ...
    def __init__(self):
        ...
        for legacy in LEGACY_SETTINGS:
            ...
            assert name not in self.attrs, (
                "legacy setting (%s) cannot also be a regular setting" % name
            )
    ...
    def external_dict(self, skip_keys={}):  # noqa
        ...
        for key, value in self.dict().items():
            if (
                ...
                and key not in INTERNAL_SETTINGS
    ...
    def __setattr__(self, name, value):
        ...
        if name in self.legacy_settings:
            ...
            if fixed_values and value not in fixed_values:
                exit_with_error(
                    f"invalid command line setting `-s{name}={value}`: {error_message}"
                )
            diagnostics.warning(
                "legacy-settings", "use of legacy setting: %s (%s)", name, error_message
            )
    ...
    def check_type(self, name, value):
        ...
        if expected_type == bool:
            ...
            if value in ("True", "False", "true", "false"):
                exit_with_error(
                    "attempt to set `%s` to `%s`; use 1/0 to set boolean settings"
                    % (name, value)
                )
        if type(value) is not expected_type:
            exit_with_error(
                "setting `%s` expects `%s` but got `%s`"
                % (name, expected_type.__name__, type(value).__name__)
            )
