...
def emscript(in_wasm, out_wasm, outfile_js, js_syms, finalize=True, base_metadata=None):
  ...
  other_exports = [e for e in other_exports if not shared.is_internal_global(e.name)]
...
def create_receiving(function_exports, other_exports, library_symbols, aliases):
  ...
  if settings.WASM_ESM_INTEGRATION:
    exported_symbols = [e.name for e in other_exports] + list(function_exports.keys())
    exports = []
  ...
  for sym, sig in exports.items():
    is_function = type(sig) == webassembly.FuncType
    mangled = asmjs_mangle(sym)
    assignment = mangled
    ...
    elif not is_function and sig.kind == webassembly.ExternType.GLOBAL:
      if settings.LEGACY_VM_SUPPORT:
        receiving.append(f"  {assignment} = typeof wasmExports['{sym}'] == 'object' ? wasmExports['{sym}'].value : wasmExports['{sym}'];")
      else:
        receiving.append(f"  {assignment} = wasmExports['{sym}'].value;")
    else:
