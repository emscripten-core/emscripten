...
def run(sys_argv):
  if DEBUG: logging.warning('invocation: ' + ' '.join(sys_argv) + (' + ' + EMCC_CFLAGS if EMCC_CFLAGS else '') + '  (in ' + os.getcwd() + ')')
  if EMCC_CFLAGS: sys_argv.append(EMCC_CFLAGS)
  ...
  EMCC_CXX = '--emscripten-cxx' in sys_argv
  sys_argv = filter(lambda x: x != '--emscripten-cxx', sys_argv)

  if len(sys_argv) <= 1 or ('--help' not in sys_argv and len(sys_argv) >= 2 and sys_argv[1] != '--version'):
  ...
  while response_file:
    ...
    for index in range(1, len(sys_argv)):
      if sys_argv[index][0] == '@':
        ...
        response_file = True
        extra_args = read_response_file(sys_argv[index])
        # slice in extra_args in place of the response file arg
        sys_argv[index:index+1] = extra_args
        break

  if len(sys_argv) == 1 or '--help' in sys_argv:
  ...
  elif len(sys_argv) == 2 and sys_argv[1] == '-v': # -v with no inputs
  ...
  CONFIGURE_CONFIG = (os.environ.get('EMMAKEN_JUST_CONFIGURE') or 'conftest.c' in sys_argv) and not os.environ.get('EMMAKEN_JUST_CONFIGURE_RECURSE')
  CMAKE_CONFIG = 'CMakeFiles/cmTryCompileExec.dir' in ' '.join(sys_argv)# or 'CMakeCCompilerId' in ' '.join(sys_argv)
  if CONFIGURE_CONFIG or CMAKE_CONFIG:
    ...
    if not ('CXXCompiler' in ' '.join(sys_argv) or EMCC_CXX):
    ...
    cmd = compiler + list(filter_emscripten_options(sys_argv[1:]))
  ...
  if EMMAKEN_CFLAGS: sys_argv += shlex.split(EMMAKEN_CFLAGS)
  ...
  if len(sys_argv) == 1 or sys_argv[1] in ['x', 't']:
    # noop ar
  ...
  for i in range(len(sys_argv)-1):
    if sys_argv[i].startswith('-o='):
      raise Exception('Invalid syntax: do not use -o=X, use -o X')

    if sys_argv[i] == '-o':
      target = sys_argv[i+1]
      sys_argv = sys_argv[:i] + sys_argv[i+2:]
      break
  ...
  try:
    ...
    for change in settings_changes:
      ...
      if value[0] == '@':
        ...
        value = value.replace('\\', '\\\\')
      shared.Settings[key] = json.loads(value)
      if key == 'EXPORTED_FUNCTIONS':
    ...
    if final_suffix not in JS_CONTAINING_SUFFIXES:
      if not specified_target:
        ...
        if len(input_files) == 1:
          ...
          assert len(original_input_files) == 1 or not has_dash_c, 'fatal error: cannot specify -o with -c with multiple files' + str(sys_argv) + ':' + str(original_input_files)
