...
void* ThreadMain(void* arg) {
  ...
  for (int iter = 0; iter < NUM_ITERS; ++iter) {
    for (int i = 0; i < NUM_KEYS; ++i) {
      ...
      //		emscripten_errf("Thread %d: Read value %d from TLS for
      //key at index %d", pthread_self(), (int)local_keys[i], i);
    }
  ...
  for (int i = 0; i < NUM_KEYS; ++i) {
    local_keys[i] = (uintptr_t)pthread_getspecific(keys[i]);
    //	emscripten_errf("Thread %d final verify: Read value %d from TLS for key
    //at index %d", pthread_self(), (int)local_keys[i], i);
