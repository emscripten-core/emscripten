...
// Stress test realloc() behavior after a suspected instability of realloc() function
// in https://groups.google.com/g/emscripten-discuss/c/xvcjByZKxVA
...
void worker_main()
{
  ...
  unsigned char byteFillPattern = (unsigned char)(emscripten_math_random()*0xFF);
  unsigned char *ptr = 0;
  size_t size = 0;
#define ALLOC_SZ 128*1024
  ...
  for(int i = 0; i < 128; ++i)
  {
    ...
    // Verify that after realloc, the new pointer should hold the old byte fill pattern
    emmalloc_validate_memory_regions();
    ...
    ++byteFillPattern;
//  EM_ASM(console.log('Worker ' + Module['$ww'] + ': doing memory fill of address 0x'+$0.toString(16) + ' size ' + $1 + ' with byte 0x'+$2.toString(16)), ptr, size, byteFillPattern);
    emmalloc_validate_memory_regions();
...
void test_done_listener()
{
  ...
  emscripten_console_log("All threads successfully finished stress testing realloc()!");
#ifdef REPORT_RESULT
  REPORT_RESULT(0);
#endif
}
...
int main()
{
  ...
  emscripten_wasm_worker_t listener = emscripten_malloc_wasm_worker(/*stack size: */1024);
