...
''' % (version, ' '.join(config.NODE_JS)))
  ...
  def test_binaryen_version(self):
    ...
    self.check_working([EMCC, test_file('hello_world.c'), '-O3'], 'error parsing binaryen version (wasm-opt version foo). Please check your binaryen installation')

    make_fake_tool(self.in_dir('fake', 'bin', 'wasm-opt'), '70')
    self.check_working([EMCC, test_file('hello_world.c'), '-O3'], 'unexpected binaryen version: 70 (expected ')
