...
static void main_loop(void) {
  ...
  // The async resolution pre-warms getaddrinfo's cache: the same name that was
  // EAI_AGAIN up front now resolves synchronously.
  struct addrinfo hints = {0};
  hints.ai_family = AF_INET;
  hints.ai_socktype = SOCK_STREAM;
  struct addrinfo* sync_res = NULL;
  if (getaddrinfo(HOST, NULL, &hints, &sync_res) != 0) fail("host should be cached");
  assert(sync_res && sync_res->ai_addr);
  freeaddrinfo(sync_res);
