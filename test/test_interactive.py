...
class interactive(BrowserCore):
  ...
  def test_html5_fullscreen(self):
    self.btest(
      ...
      args=[
        ...
        '-sEXPORTED_FUNCTIONS=_requestFullscreen,_enterSoftFullscreen,_main',
        '--shell-file',
  ...
  def test_sdl_audio_mix_channels_halt(self):
    ...
    self.btest_exit(
      'test_sdl_audio_mix_channels_halt.c', args=['-O2', '--minify=0', '--preload-file', 'the_entertainer.ogg']
    )
  ...
  def test_sdl_audio_panning(self):
    ...
    self.btest_exit(
      ...
      args=[
        ...
        '--preload-file',
        'the_entertainer.wav',
        '-sEXPORTED_FUNCTIONS=_main,_play',
      ],
    )
  ...
  def test_sdl_audio_beeps(self):
    # use closure to check for a possible bug with closure minifying away newer Audio() attributes
    self.btest_exit(
      'test_sdl_audio_beep.cpp',
      args=['-O2', '--closure=1', '--minify=0', '-sDISABLE_EXCEPTION_CATCHING=0', '-o', 'page.html'],
    )

  def test_sdl2_mixer_wav(self):
    shutil.copy(test_file('sounds/the_entertainer.wav'), 'sound.wav')
    self.btest_exit(
      'browser/test_sdl2_mixer_wav.c',
      args=['-O2', '-sUSE_SDL=2', '-sUSE_SDL_MIXER=2', '-sINITIAL_MEMORY=33554432', '--preload-file', 'sound.wav'],
    )
  ...
  def test_sdl2_audio_beeps(self):
    ...
    self.btest_exit(
      'browser/test_sdl2_audio_beep.cpp',
      args=['-O2', '--closure=1', '--minify=0', '-sDISABLE_EXCEPTION_CATCHING=0', '-sUSE_SDL=2'],
    )
  ...
  def test_openal_buffers(self):
    self.btest_exit(
      'openal/test_openal_buffers.c',
      args=['--preload-file', test_file('sounds/the_entertainer.wav') + '@/'],
    )

  def test_openal_buffers_animated_pitch(self):
    self.btest_exit(
      'openal/test_openal_buffers.c',
      args=['-DTEST_ANIMATED_PITCH=1', '--preload-file', test_file('sounds/the_entertainer.wav') + '@/'],
    )

  def test_openal_looped_pitched_playback(self):
    self.btest(
      'openal/test_openal_playback.c',
      '1',
      args=['-DTEST_LOOPED_PLAYBACK=1', '--preload-file', test_file('sounds/the_entertainer.wav') + '@/audio.wav'],
    )
  ...
  def test_openal_looped_seek_playback(self):
    self.btest(
      ...
      '1',
      args=[
        '-DTEST_LOOPED_SEEK_PLAYBACK=1',
        '-DTEST_LOOPED_PLAYBACK=1',
        '--preload-file',
  ...
  def test_openal_animated_looped_pitched_playback(self):
    self.btest(
      ...
      '1',
      args=[
        '-DTEST_ANIMATED_LOOPED_PITCHED_PLAYBACK=1',
        '-DTEST_LOOPED_PLAYBACK=1',
        '--preload-file',
        test_file('sounds/the_entertainer.wav') + '@/audio.wav',
      ],
  ...
  def test_openal_animated_looped_distance_playback(self):
    self.btest(
      ...
      '1',
      args=[
        '-DTEST_ANIMATED_LOOPED_DISTANCE_PLAYBACK=1',
        '-DTEST_LOOPED_PLAYBACK=1',
        '--preload-file',
        test_file('sounds/the_entertainer.wav') + '@/audio.wav',
      ],
  ...
  def test_openal_animated_looped_doppler_playback(self):
    self.btest(
      ...
      '1',
      args=[
        '-DTEST_ANIMATED_LOOPED_DOPPLER_PLAYBACK=1',
        '-DTEST_LOOPED_PLAYBACK=1',
        '--preload-file',
        test_file('sounds/the_entertainer.wav') + '@/audio.wav',
      ],
  ...
  def test_openal_animated_looped_panned_playback(self):
    self.btest(
      ...
      '1',
      args=[
        '-DTEST_ANIMATED_LOOPED_PANNED_PLAYBACK=1',
        '-DTEST_LOOPED_PLAYBACK=1',
        '--preload-file',
        test_file('sounds/the_entertainer.wav') + '@/audio.wav',
      ],
  ...
  def get_freealut_library(self):
    ...
    if WINDOWS and shutil.which('cmake'):
      return self.get_library(
        os.path.join('third_party', 'freealut'),
        'libalut.a',
        configure=['cmake', '.'],
  ...
  def test_cpuprofiler_memoryprofiler(self):
    self.btest(
      'hello_world_gles.c',
      expected='0',
      args=['-DLONGTEST=1', '-DTEST_MEMORYPROFILER_ALLOCATIONS_MAP=1', '-O2', '--cpuprofiler', '--memoryprofiler'],
    )
  ...
  def test_html5_callbacks_on_calling_thread(self):
    ...
    for args in ([], ['-DTEST_SYNC_BLOCKING_LOOP=1']):
      self.btest(
        ...
        expected='1',
        args=args + ['-sDISABLE_DEPRECATED_FIND_EVENT_TARGET_BEHAVIOR', '-pthread', '-sPROXY_TO_PTHREAD'],
      )
  ...
  def test_webgl_offscreen_canvas_in_two_pthreads(self, args):
    self.btest(
      'gl_in_two_pthreads.c', expected='1', args=args + ['-pthread', '-lGL', '-sGL_DEBUG', '-sPROXY_TO_PTHREAD']
    )
  ...
  def test_audio_worklet(self):
    self.btest(
      'webaudio/audioworklet.c',
      expected='0',
      args=['-sAUDIO_WORKLET', '-sWASM_WORKERS', '--preload-file', test_file('hello_world.c') + '@/'],
    )
  ...
  def test_audio_worklet_emscripten_futex_wake(self):
    self.btest(
      'webaudio/audioworklet_emscripten_futex_wake.cpp',
      expected='0',
      args=['-sAUDIO_WORKLET', '-sWASM_WORKERS', '-pthread', '-sPTHREAD_POOL_SIZE=2'],
    )
  ...
  def test_audio_worklet_modularize(self):
    self.btest(
      'webaudio/audioworklet.c',
      expected='0',
      args=['-sAUDIO_WORKLET', '-sWASM_WORKERS', '-sMINIMAL_RUNTIME', '-sMODULARIZE'],
    )
