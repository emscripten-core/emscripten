...
int main() {
  ...
  errno = 0;
  mkdirTree("/working", 0777);
  mkdirTree("/foobar", 01777);
  printf("Errno: %s\n", strerror(errno));
  assert(errno == 0);
  ...
  errno = 0;
  mkdirTree("/dev/stdout/fake-directory", 0777);
  ...
  errno = 0;
  mkdirTree("/dev/false-path/fake-directory", 0777);
  printf("Errno: %s\n", strerror(errno));
  assert(errno == ENOENT);

  // Try to make a directory under the `working` directory.
  errno = 0;
  mkdirTree("/working/new-directory", 0777);
  printf("Errno: %s\n", strerror(errno));
  assert(errno == 0);
