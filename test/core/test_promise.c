...
static em_promise_result_t test_create(void** result, void* data, void* value) {
  ...
  emscripten_promise_resolve(
    match_fulfilled, EM_PROMISE_MATCH, fulfilled.handle);
  ...
  emscripten_promise_resolve(match_rejected, EM_PROMISE_MATCH, rejected.handle);
...
test_promisify(void** result, void* data, void* value) {
  ...
  emscripten_async_call(called_async, promise.handle, 0);
