...
def run_benchmark(benchmark_file, results_file, build_args):
  ...
  html = (
    '''<html><head></head><body><h1>SSE JavaScript Benchmark</h1>
  ...
  for chart_name, chart_native_results in charts_native.items():
    ...
    for result in native_results:
      ...
      textual_results_native += (
        'Native ' + result['category'] + ': ' + "{:10.4f}".format(nsc) + 'ns -> ' + "{:10.4f}".format(nsi) + 'ns. '
      )
      textual_results_native += (
        'Native SSE is ' + format_comparison(nsi, nsc) + ' than native scalar. &nbsp; &nbsp; &nbsp; &nbsp; <br />'
      )
      ...
      if html_result is not None:
        ...
        textual_results_html += (
          'JS ' + result['category'] + ': ' + "{:10.4f}".format(hsc) + 'ns -> ' + "{:10.4f}".format(hsi) + 'ns. '
        )
        textual_results_html += (
          'JS SSE is ' + format_comparison(hsi, hsc) + ' than JS scalar. &nbsp; &nbsp; &nbsp; &nbsp; <br />'
        )
        textual_results_html2 += (
          'JS '
          + result['category']
          + ': JS scalar is '
          + format_comparison(hsc, nsc)
    ...
    html += (
      ...
      + '''
            ...
            series: [{
                name: 'Native scalar',
                data: ['''
      + ','.join(nativeScalarResults)
      + ''']

            }, {
                name: 'Native SSE',
                data: ['''
      + ','.join(nativeSimdResults)
      + ''']

            }, {
                name: 'JS scalar',
                data: ['''
      + ','.join(htmlScalarResults)
      + ''']

            }, {
                name: 'JS SSE',
                data: ['''
      + ','.join(htmlSimdResults)
      + ''']
  ...
  html += (
    ...
    + ''']
                ...
                name: 'Native SSE',
                data: ['''
    + (str(total_time_native_simd / total_time_native_scalar) if total_time_native_scalar != 0 else 'N/A')
    + ''']

            }, {
                name: 'JS scalar',
                data: ['''
    + (str(total_time_html_scalar / total_time_native_scalar) if total_time_native_scalar != 0 else 'N/A')
    + ''']

            }, {
                name: 'JS SSE',
                data: ['''
    + (str(total_time_html_simd / total_time_native_scalar) if total_time_native_scalar != 0 else 'N/A')
    + ''']
