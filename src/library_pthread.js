...
var LibraryPThread = {
  ...
  emscripten_futex_wait: function(addr, val, timeout) {
    ...
    if (!ENVIRONMENT_IS_WEB) {
      ...
      // nonzero, the contents of the address pointed by _main_thread_futex
      ...
      // two (there is just a single _main_thread_futex at a time), unmark
      ...
      // between when we are not set as the value of _main_thread_futex.
#if ASSERTIONS
      assert(_main_thread_futex > 0);
#endif
      var lastAddr = Atomics.exchange(HEAP32, _main_thread_futex >> 2, addr);
#if ASSERTIONS
      ...
      while (1) {
        ...
        if (tNow > tEnd) {
          ...
          lastAddr = Atomics.exchange(HEAP32, _main_thread_futex >> 2, 0);
#if ASSERTIONS
        ...
        //    which waits on futex B. that sets the _main_thread_futex address to
        //    futex B, and there is no longer any mention of futex A.
        //  * a worker is done with futex A. it checks _main_thread_futex but does
        ...
        //  * we return to the wait on futex A. _main_thread_futex is 0, but that
        //    is because of futex B being done - we can't tell from
        //    _main_thread_futex whether A is done or not. therefore, check the
        ...
        // address in _main_thread_futex around calls to
        ...
        lastAddr = Atomics.exchange(HEAP32, _main_thread_futex >> 2, addr);
#if ASSERTIONS
        assert(lastAddr == 0);
#endif
      }
  ...
  emscripten_futex_wake: function(addr, count) {
    ...
    assert(_main_thread_futex > 0);
#endif
    var mainThreadWaitAddress = Atomics.load(HEAP32, _main_thread_futex >> 2);
    var mainThreadWoken = 0;
    if (mainThreadWaitAddress == addr) {
#if ASSERTIONS
      // We only use _main_thread_futex on the main browser thread, where we
...
#endif
      var loadedAddr = Atomics.compareExchange(HEAP32, _main_thread_futex >> 2, mainThreadWaitAddress, 0);
