...
function SAFE_HEAP_INDEX(arr, idx, log) {
#if CAN_ADDRESS_2GB
  idx >>>= 0;
#endif
...
#if SAFE_HEAP_LOG
  dbg(`SAFE_HEAP ${log}: ${[arr.constructor.name, idx, SAFE_HEAP_COUNTER++]}`);
#endif
  if (idx <= 0) abort(`segmentation fault ${log} ${bytes} bytes from address ${dest}`);
#if EXIT_RUNTIME
...
#endif
    var brk = _sbrk(0);
    if (dest + bytes > brk) abort(`segmentation fault, exceeded the top of the available dynamic heap when ${log} ${bytes} bytes from address ${dest}. DYNAMICTOP=${brk}`);
