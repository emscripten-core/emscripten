...
// The socket syscalls (socket/bind/connect/accept4/recv*/send*/... and the
// $getSocketFromFD / $getSocketAddress helpers) live in libsockfs.js: they are
// SOCKFS-coupled, and the blocking data ops build on $streamWaitOp there.
// (Under PROXY_POSIX_SOCKETS / WASMFS they are implemented natively instead.)
  __syscall_fchdir: (fd) => {
    var stream = SYSCALLS.getStreamFromFD(fd);
