...
*/
  ...
  glGetUniformLocation: (program, name) => {
    ...
    if (program = GL.programs[program]) {
      ...
      if (leftBrace > 0) {
  #if GL_ASSERTIONS
        assert(name.slice(leftBrace + 1).length == 1 || !isNaN(parseInt(name.slice(leftBrace + 1))), `Malformed input parameter name "${name}" passed to glGetUniformLocation!`);
  #endif
        arrayIndex = parseInt(name.slice(leftBrace + 1)) >>> 0; // "index]", coerce parseInt(']') with >>>0 to treat "foo[]" as "foo[0]" and foo[-1] as unsigned out-of-bounds.
  ...
  glShaderSource: (shader, count, string, length) => {
    ...
    while (match = regex.exec(source)) {
      ...
      console.dir(match);
#endif
      explicitUniformLocations[match[5]] = parseInt(match[1]);
#if GL_TRACK_ERRORS
    ...
    while (bindingMatch = bindingRegex.exec(source)) {
      ...
      for (var i = bindingMatch.index; i < source.length && source[i] != ';'; ++i) {
        if (source[i] == '[') {
          arrayLength = parseInt(source.slice(i+1));
          break;
        }
...
#endif
      var binding = parseInt(bindingMatch[1]);
#if GL_TRACK_ERRORS
