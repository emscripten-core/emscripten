...
addToLibrary({
  ...
  $SOCKFS: {
    ...
    // Mark an async-completion pseudo-socket ready: flip it readable and deliver
    // its message callback. Skipped if the fd was closed (and possibly recycled)
    // before completion. This (with the sock.dns hooks in stream_ops poll/close)
    // is the generic pollable-completion seam: a future ring/aio completion fd
    // would reuse the same readiness mechanism rather than re-adding one.
    finishDns(sock) {
      sock.dnsDone = true;
      if (SOCKFS.getSocket(sock.stream.fd) === sock) {
  ...
  // Returns a pollable fd that becomes readable (and delivers the socket message
  // callback) when resolution completes; read the result with
  // emscripten_dns_lookup_result. Returns -1 on failure to allocate the fd.
  ...
  emscripten_dns_lookup_async: (node, service, hint) => {
    ...
    if (typeof desc === 'object' && !desc.entries) {
      ...
      nodeSockOps.resolveAddrInfo(desc).then((eai) => {
        sock.dnsResult = eai;
        sock.dnsDesc = desc;
