...
var NodeSockFSLibrary = {
  ...
  $nodeSockOps: {
    ...
    getUtil() {
      return nodeSockOps.utilModule ??= (process.getBuiltinModule || require)('util');
    },
    getDgram() {
    ...
    useDgram() {
      var proto = nodeSockOps.getDgram().Socket.prototype;
      return nodeSockOps.dgramSync ??=
        typeof proto.bindSync == 'function' && typeof proto.connectSync == 'function';
    },
    // Queue a received datagram and signal readiness. A connected datagram
    // socket only accepts datagrams from its peer. Shared by both backends.
    ...
    errnoForCode(code) {
      var name = nodeSockOps.getUtil().getSystemErrorName(code);
    ...
    bindHandle(sock, addr, port) {
      ...
      throw new FS.ErrnoError(nodeSockOps.errnoForCode(code));
    },
    ...
    ensureUdpHandle(sock) {
      ...
      if (nodeSockOps.useDgram()) {
        var socket = nodeSockOps.getDgram().createSocket(sock.family === {{{ cDefs.AF_INET6 }}} ? 'udp6' : 'udp4');
        socket.on('message', (msg, rinfo) => {
          var data = new Uint8Array(msg.length);
          data.set(msg);
          nodeSockOps.udpDeliver(sock, rinfo.address, rinfo.port, data);
        });
        socket.on('error', (e) => {
          sock.error = nodeSockOps.errnoForNode(e);
      ...
      handle.onmessage = (nread, _h, buf, rinfo) => {
        if (nread < 0) {
          sock.error = nodeSockOps.errnoForCode(nread);
    ...
    applyOptions(sock) {
      ...
      nodeSockOps.applyKeepAlive(sock);
    },
    ...
    bind(sock, addr, port) {
      ...
      if (sock.type === {{{ cDefs.SOCK_DGRAM }}}) {
        var udp = nodeSockOps.ensureUdpHandle(sock);
        if (sock.udpPublic) {
          var a;
          ...
          if (ucode) throw new FS.ErrnoError(nodeSockOps.errnoForCode(ucode));
          var uname = {};
      ...
      nodeSockOps.bindHandle(sock, addr, port);
      sock.state = 'bound';
    },
    connect(sock, addr, port) {
      ...
      conn.once('connect', () => {
        ...
        nodeSockOps.applyOptions(sock);
        SOCKFS.emit('open', sock.stream.fd);
      });
      nodeSockOps.wireConnection(sock, conn);
      conn.connect({ host: addr, port, lookup: nodeSockOps.noLookup });
    },
    listen(sock, backlog) {
      ...
      if (!sock.bound) {
        nodeSockOps.bindHandle(sock, '0.0.0.0', 0);
        sock.state = 'bound';
      }
      var server = new (nodeSockOps.getNet().Server)({ pauseOnConnect: true, allowHalfOpen: true });
      sock.server = server;
      sock.state = 'listen';
      server.on('connection', (conn) => {
        ...
        nodeSockOps.wireConnection(newsock, conn);
    ...
    sendmsg(sock, buffer, offset, length, addr, port) {
      if (sock.type === {{{ cDefs.SOCK_DGRAM }}}) {
        ...
        if (sock.udpPublic) nodeSockOps.startUdpRecv(sock);
        ...
        if (sock.udpPublic) {
          ...
          if (code < 0) throw new FS.ErrnoError(nodeSockOps.errnoForCode(code));
      ...
      try {
        ...
        throw new FS.ErrnoError(nodeSockOps.errnoForNode(e));
      }
    ...
    getsockopt(sock, level, optname, optval, optlen) {
      ...
      if (level === {{{ cDefs.SOL_SOCKET }}}) {
        switch (optname) {
          ...
          case 8: val = nodeSockOps.udpBufferSize(sock, true) ?? (sock.opts.recvBuf || 65536); break;
          case 7: val = nodeSockOps.udpBufferSize(sock, false) ?? (sock.opts.sendBuf || 65536); break;
