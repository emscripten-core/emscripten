...
// Pthreads need their clocks synchronized to the execution of the main thread, so give them a special form of the function.
  ...
  /* Different functions related to calling or binding a function pointer in Wasm from JS:
    1. _dynCallI32Pair(): internal, do not call in user code.
    2. dynCall(sig, ptr, argsArray): public user facing function, dynamic signature
         dispatch with i64s as a i32 pair.
    3. dynCall_xxx(ptr, arg0, arg1, ...): same as above, i64s as i32 pair, but with static dispatch.
  ...
  */
  ...
  // Used in library code to get JS function from wasm function pointer, using
  // the i64 as a i32 pair calling convention.
  $getDynCaller__deps: ['$dynCall'],
  $getDynCaller: function(sig, ptr) {
#if DYNCALLS || !WASM_BIGINT

#if SHRINK_LEVEL < 2
    // Speed micro-optimization: use wasmTable if not calling a 64-bit signature.
  ...
  // dynCall: Call a JS function using the i64 as a i32 pair calling convention.
#if DYNCALLS || !WASM_BIGINT
  $dynCall__deps: ['_dynCallI32Pair', '$wbind'],
#else
  $dynCall__deps: ['$wbind'],
#endif
...
#else
    // In -Oz builds, always dispatch via the dynCall functions generated by
