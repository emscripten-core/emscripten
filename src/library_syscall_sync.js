...
if (!WASM_BACKEND) {
  for (var x in WASI_SYSCALLS) {
    SyscallsSyncLibrary['__wasi_' + x] = x;
  }
}
...
for (var x in SyscallsSyncLibrary) {
  ...
  SyscallsSyncLibrary[x] = eval('(' + t + ')');
  if (!SyscallsSyncLibrary[x + '__deps']) SyscallsSyncLibrary[x + '__deps'] = [];
  SyscallsSyncLibrary[x + '__deps'].push('$SYSCALLS');
#if USE_PTHREADS
  // proxy all syscalls synchronously, for their return values
  SyscallsSyncLibrary[x + '__proxy'] = 'sync';
#endif
}
