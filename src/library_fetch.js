var LibraryFetch = {
  ...
  _emscripten_fetch_load_cached_data: function(db, fetch, onsuccess, onerror) {
    if (!db) {
#if FETCH_DEBUG
  ...
  _emscripten_fetch_cache_data: function(db, fetch, data, onsuccess, onerror) {
    if (!db) {
#if FETCH_DEBUG
      console.error('fetch: IndexedDB not available!');
#endif
  ...
  _emscripten_fetch_xhr: function(fetch, onsuccess, onerror, onprogress) {
  ...
  emscripten_start_fetch: function(fetch) {
