...

{
  ...
  "RUNTIME_LOGGING": 0, // Whether extra logging should be enabled.
                        // This logging isn't quite assertion-quality in that it isn't necessarily a symptom that something is wrong.
  "STACK_OVERFLOW_CHECK": 0, // Chooses what kind of stack smash checks to emit to generated code:
  ...
  "VERBOSE": 0, // When set to 1, will generate more verbose output during compilation.

  "INVOKE_RUN": 1, // Whether we will run the main() function. Disable if you embed the generated
                   // code in your own, and will call main() yourself at the right time (which you
                   // can do with Module.callMain(), with an optional parameter of commandline args).
  "NO_EXIT_RUNTIME": 1, // If 1, the runtime is not quit when main() completes (allowing code to
                        ...
                        // on exit.
  "MEM_INIT_METHOD": 0, // How to represent the initial memory content.
  ...
  "TOTAL_STACK": 5242880, // The total stack size. There is no way to enlarge the stack, so this
                          // value must be large enough for the program's requirements. If
                          // assertions are on, we will assert on not exceeding this, otherwise,
                          // it will fail silently.
  "TOTAL_MEMORY": 16777216, // The total amount of memory to use. Using more memory than this will
                            // cause us to expand the heap, which can be costly with typed arrays:
                            // we need to copy the old heap into a new one in that case.
  "MALLOC": "dlmalloc", // What malloc()/free() to use, out of
  ...
  "ABORTING_MALLOC": 1, // If 1, then when malloc would fail we abort(). This is nonstandard behavior,
  ...
  "ALLOW_MEMORY_GROWTH": 0, // If false, we abort with an error if we try to allocate more memory than
  ...
  "GLOBAL_BASE": -1, // where global data begins; the start of static memory. -1 means use the
                     // default, any other value will be used as an override
  "STACK_START": -1, // where the stack will begin. -1 means use the default. if the stack cannot
  ...
  // Code embetterments
  "DOUBLE_MODE": 1, // How to load and store 64-bit doubles.
  ...
  "UNALIGNED_MEMORY": 0, // If enabled, all memory accesses are assumed to be unaligned.  In unaligned memory mode,
  ...
  "FORCE_ALIGNED_MEMORY": 0, // If enabled, assumes all reads and writes are fully aligned for the type they
  ...
  "WARN_UNALIGNED": 0, // Warn at compile time about instructions that LLVM tells us are not fully aligned.
  ...
  "PRECISE_F32": 0, // 0: Use JS numbers for floating-point values. These are 64-bit and do not model C++
  ...
  "SIMD": 0, // Whether to allow autovectorized SIMD code ( https://github.com/johnmccutchan/ecmascript_simd ).
  ...
  "USE_CLOSURE_COMPILER": 0, // Whether closure compiling is being run on this output

  "IGNORE_CLOSURE_COMPILER_ERRORS": 0, // Ignore closure warnings and errors (like on duplicate definitions)
  ...
  "INLINING_LIMIT": 0, // A limit on inlining. If 0, we will inline normally in LLVM and
                       ...
                       // inlining
  "OUTLINING_LIMIT": 0, // A function size above which we try to automatically break up
  ...
  "AGGRESSIVE_VARIABLE_ELIMINATION": 0, // Run aggressiveVariableElimination in js-optimizer.js
  "SIMPLIFY_IFS": 1, // Whether to simplify ifs in js-optimizer.js

  // Generated code debugging options
  "SAFE_HEAP": 0, // Check each write to the heap, for example, this will give a clear
  ...
  "ALIASING_FUNCTION_POINTERS": 0, // Whether to allow function pointers to alias if they have
  ...
  "EMULATED_FUNCTION_POINTERS": 0, // asm.js:
                                   ...
                                   // yet to work around.
  "EMULATE_FUNCTION_POINTER_CASTS": 0, // Allows function pointers to be cast, wraps each
  ...
  "EXCEPTION_DEBUG": 0, // Print out exceptions in emscriptened code. Does not work in asm.js mode

  "DEMANGLE_SUPPORT": 0, // If 1, build in libcxxabi's full c++ demangling code, to allow stackTrace()
                         // to emit fully proper demangled c++ names

  "LIBRARY_DEBUG": 0, // Print out when we enter a library call (library*.js). You can also unset
                      // Runtime.debug at runtime for logging to cease, and can set it when you
                      // want it back. A simple way to set it in C++ is
                      //   emscripten_run_script("Runtime.debug = ...;");
  "SYSCALL_DEBUG": 0, // Print out all syscalls
  "SOCKET_DEBUG": 0, // Log out socket/network data transfer.
  "SOCKET_WEBRTC": 0, // Select socket backend, either webrtc or websockets. XXX webrtc is not currently tested, may be broken
  ...
  "WEBSOCKET_URL": "ws://", // A string containing either a WebSocket URL prefix (ws:// or wss://) or a complete
  ...
  "WEBSOCKET_SUBPROTOCOL": "binary", // A string containing a comma separated list of WebSocket subprotocols
                                     // as would be present in the Sec-WebSocket-Protocol header.

  "OPENAL_DEBUG": 0, // Print out debugging information from our OpenAL implementation.

  "GL_ASSERTIONS": 0, // Adds extra checks for error situations in the GL library. Can impact performance.
  "TRACE_WEBGL_CALLS": 0, // If enabled, prints out all API calls to WebGL contexts. (*very* verbose)
  "GL_DEBUG": 0, // Enables more verbose debug printing of WebGL related operations. As with LIBRARY_DEBUG, this is toggleable at runtime with option GL.debug.
  "GL_TESTING": 0, // When enabled, sets preserveDrawingBuffer in the context, to allow tests to work (but adds overhead)
  "GL_MAX_TEMP_BUFFER_SIZE": 2097152, // How large GL emulation temp buffers are
  "GL_UNSAFE_OPTS": 1, // Enables some potentially-unsafe optimizations in GL emulation code
  "FULL_ES2": 0,   // Forces support for all GLES2 features, not just the WebGL-friendly subset.
  "USE_WEBGL2": 0, // Enables WebGL2 native functions. This mode will also create a WebGL2
                   // context by default if no version is specified.
  "WEBGL2_BACKWARDS_COMPATIBILITY_EMULATION": 0, // If true, emulates some WebGL 1 features on WebGL 2 contexts, meaning that applications that
  ...
  "FULL_ES3": 0,   // Forces support for all GLES3 features, not just the WebGL2-friendly subset.
  "LEGACY_GL_EMULATION": 0, // Includes code to emulate various desktop GL features. Incomplete but useful
  // in some cases, see http://kripken.github.io/emscripten-site/docs/porting/multimedia_and_graphics/OpenGL-support.html
  "GL_FFP_ONLY": 0, // If you specified LEGACY_GL_EMULATION = 1 and only use fixed function pipeline in your code,
  ...
  "GL_PREINITIALIZED_CONTEXT": 0, // If you want to create the WebGL context up front in JS code, set this to 1 and set Module['preinitializedWebGLContext']
                                  // to a precreated WebGL context. WebGL initialization afterwards will use this GL context to render.
  "STB_IMAGE": 0, // Enables building of stb-image, a tiny public-domain library for decoding images, allowing
  ...
  "LEGACY_VM_SUPPORT": 0, // Enable this to get support for non-modern browsers, node.js, etc. This gives you
  ...
  "ENVIRONMENT": "", // By default, emscripten output will run on the web, in a web worker,
  ...
  "LZ4": 0, // Enable this to support lz4-compressed file packages. They are stored compressed in memory, and
  ...
  "DISABLE_EXCEPTION_CATCHING": 1, // Disables generating code to actually catch exceptions. This disabling is on
  ...
  "EXCEPTION_CATCHING_WHITELIST": [], // Enables catching exception in the listed functions only, if
                                      // DISABLE_EXCEPTION_CATCHING = 2 is set

  "NODEJS_CATCH_EXIT": 1, // By default we handle exit() in node, by catching the Exit exception. However,
  ...
  "ASYNCIFY": 0, // Whether to enable asyncify transformation
                 // This allows to inject some async functions to the C code that appear to be sync
                 // e.g. emscripten_sleep
  "ASYNCIFY_FUNCTIONS": ["emscripten_sleep", // Functions that call any function in the list, directly or indirectly
                         "emscripten_wget",  // will be transformed
                         "emscripten_yield"],
  "ASYNCIFY_WHITELIST": ["qsort",    // Functions in this list are never considered async, even if they appear in ASYNCIFY_FUNCTIONS
  ...
  "EXPORTED_RUNTIME_METHODS": [], // Runtime elements that are exported on Module by default. We used to export quite a lot here,
  ...
  "EXTRA_EXPORTED_RUNTIME_METHODS": [], // Additional methods to those in EXPORTED_RUNTIME_METHODS. Adjusting that list
  ...
  "FS_LOG": 0, // Log all FS operations.  This is especially helpful when you're porting
               // a new project and want to see a list of file system operations happening
               // so that you can create a virtual file system with all of the required files.
  "CASE_INSENSITIVE_FS": 0, // If set to nonzero, the provided virtual filesystem if treated case-insensitive, like
                            // Windows and OSX do. If set to 0, the VFS is case-sensitive, like on Linux.
  "MEMFS_APPEND_TO_TYPED_ARRAYS": 0, // If set to nonzero, MEMFS will always utilize typed arrays as the backing store
  ...
  "NO_FILESYSTEM": 0, // If set, does not build in any filesystem support. Useful if you are just doing pure
                      ...
                      // FORCE_FILESYSTEM
  "FORCE_FILESYSTEM": 0, // Makes full filesystem support be included, even if statically it looks like it is not
                         // used. For example, if your C code uses no files, but you include some JS that does,
                         // you might need this.
  "NODERAWFS": 0, // This mode is intended for use with Node.js (and will throw if the build runs in other engines).
  ...
  "EXPORTED_FUNCTIONS": ["_main"], // Functions that are explicitly exported. These functions are kept alive
  ...
  "EXPORT_ALL": 0, // If true, we export all the symbols. Note that this does *not* affect LLVM, so it can
                   // still eliminate functions as dead. This just exports them on the Module object.
  "EXPORT_BINDINGS": 0, // Export all bindings generator functions (prefixed with emscripten_bind_). This
                        // is necessary to use the WebIDL binder with asm.js
  "EXPORT_FUNCTION_TABLES": 0, // If true, export all the functions appearing in a function table, and the
                               // tables themselves.
  "RETAIN_COMPILER_SETTINGS": 0, // Remembers the values of these settings, and makes them accessible
                                 // through Runtime.getCompilerSetting and emscripten_get_compiler_setting.
                                 // To see what is retained, look for compilerSettings in the generated code.


  "EMSCRIPTEN_VERSION": "", // this will contain the emscripten version. you should not modify it. This
                            // and the following few settings are useful in combination with
                            // RETAIN_COMPILER_SETTINGS
  "OPT_LEVEL": 0, // this will contain the optimization level (-Ox). you should not modify it.
  "DEBUG_LEVEL": 0, // this will contain the debug level (-gx). you should not modify it.
  "PROFILING_FUNCS": 0, // Whether we are profiling functions. you should not modify it.
  ...
  "LIBRARY_DEPS_TO_AUTOEXPORT": ["memcpy"], // This list is also used to determine
  ...
  "INCLUDE_FULL_LIBRARY": 0, // Include all JS library functions instead of the sum of
  ...
  "SHELL_FILE": 0, // set this to a string to override the shell file used

  "RELOCATABLE": 0, // If set to 1, we emit relocatable code from the LLVM backend; both
                    // globals and function pointers are all offset (by gb and fp, respectively)

  "MAIN_MODULE": 0, // A main module is a file compiled in a way that allows us to link it to
                    ...
                    //     it is kept alive.
  "SIDE_MODULE": 0, // Corresponds to MAIN_MODULE (also supports modes 1 and 2)

  "RUNTIME_LINKED_LIBS": [], // If this is a main module (MAIN_MODULE == 1), then
                             // we will link these at runtime. They must have been built with
                             // SIDE_MODULE == 1.
  "BUILD_AS_SHARED_LIB": 0, // (deprecated option TODO: remove)

  "BUILD_AS_WORKER": 0, // If set to 1, this is a worker library, a special kind of library
                        // that is run in a worker. See emscripten.h

  "PROXY_TO_WORKER": 0, // If set to 1, we build the project into a js file that will run
                        // in a worker, and generate an html file that proxies input and
                        // output to/from it.
  "PROXY_TO_WORKER_FILENAME": "", // If set, the script file name the main thread loads.
                                  // Useful if your project doesn't run the main emscripten-
                                  // generated script immediately but does some setup before

  "PROXY_TO_PTHREAD": 0, // If set to 1, compiles in a small stub main() in between the real main()
  ...
  "LINKABLE": 0, // If set to 1, this file can be linked with others, either as a shared
  ...
  "STRICT": 0, // Emscripten 'strict' build mode: Drop supporting any deprecated build options.
  ...
  "WARN_ON_UNDEFINED_SYMBOLS": 1, // If set to 1, we will warn on any undefined symbols that
  ...
  "ERROR_ON_UNDEFINED_SYMBOLS": 0, // If set to 1, we will give a compile-time error on any
  ...
  "ERROR_ON_MISSING_LIBRARIES": 0, // If set to 1, any -lfoo directives pointing to nonexisting
  ...
  "SYSTEM_JS_LIBRARIES": [], // Specifies a list of Emscripten-provided JS libraries to link against.
                             // (internal, use -lfoo or -lfoo.js to link to Emscripten system JS libraries)

  "SMALL_XHR_CHUNKS": 0, // Use small chunk size for binary synchronous XHR's in Web Workers.
                         // Used for testing.
                         // See test_chunked_synchronous_xhr in runner.py and library.js.

  "HEADLESS": 0, // If 1, will include shim code that tries to 'fake' a browser
  ...
  "DETERMINISTIC": 0, // If 1, we force Date.now(), Math.random, etc. to return deterministic
                      // results. Good for comparing builds for debugging purposes (and nothing else)

  "MODULARIZE": 0, // By default we emit all code in a straightforward way into the output
                   ...
                   // for convenience.
  "MODULARIZE_INSTANCE": 0, // Similar to MODULARIZE, but while that mode exports a function,
                            ...
                            // yourself).
  "EXPORT_ES6": 0, // Export using an ES6 Module export rather than a UMD export.
                   // MODULARIZE must be enabled for ES6 exports.

  "BENCHMARK": 0, // If 1, will just time how long main() takes to execute, and not
                  // print out anything at all whatsoever. This is useful for benchmarking.

  "ASM_JS": 1, // If 1, generate code in asm.js format. If 2, emits the same code except
               // for omitting 'use asm'
  "FINALIZE_ASM_JS": 1, // If 1, will finalize the final emitted code, including operations
                        // that prevent later js optimizer passes from running, like
                        // converting +5 into 5.0 (the js optimizer sees 5.0 as just 5).

  "SWAPPABLE_ASM_MODULE": 0, // If 1, then all exports from the asm/wasm module will be accessed
  ...
  "SEPARATE_ASM": 0, // see emcc --separate-asm

  "ONLY_MY_CODE": 0, // This disables linking and other causes of adding extra code
  ...
  "PGO": 0, // Enables profile-guided optimization in the form of runtime checks for
  ...
  "DEAD_FUNCTIONS": [], // JS library functions on this list are not converted to JS, and calls to
  ...
  "EXPLICIT_ZEXT": 0, // If 1, generate an explicit conversion of zext i1 to i32, using ?:

  "EXPORT_NAME": "Module", // Global variable to export the module as for environments without a standardized module
                           // loading system (e.g. the browser and SM shell).

  "NO_DYNAMIC_EXECUTION": 0, // When set to 1, we do not emit eval() and new Function(), which disables some functionality
  ...
  "EMTERPRETIFY": 0, // Runs tools/emterpretify on the compiler output
  "EMTERPRETIFY_FILE": "", // If defined, a file to write bytecode to, otherwise the default is to embed it in text JS arrays (which is less efficient).
  ...
  "EMTERPRETIFY_BLACKLIST": [], // Functions to not emterpret, that is, to run normally at full speed
  "EMTERPRETIFY_WHITELIST": [], // If this contains any functions, then only the functions in this list
                                // are emterpreted (as if all the rest are blacklisted; this overrides the BLACKLIST)
  "EMTERPRETIFY_ASYNC": 0, // Allows sync code in the emterpreter, by saving the call stack, doing an async delay, and resuming it
  "EMTERPRETIFY_ADVISE": 0, // Performs a static analysis to suggest which functions should be run in the emterpreter, as it
  ...
  "EMTERPRETIFY_SYNCLIST": [], // If you have additional custom synchronous functions, add them to this list and the advise mode
                               // will include them in its analysis.

  "SPLIT_MEMORY": 0, // If > 0, we split memory into chunks, of the size given in this parameter.
  ...
  "SAFE_SPLIT_MEMORY": 0, // Similar to SAFE_HEAP, but for SPLIT_MEMORY.

  "RUNNING_JS_OPTS": 0, // whether js opts will be run, after the main compiler
  "BOOTSTRAPPING_STRUCT_INFO": 0, // whether we are in the generate struct_info bootstrap phase
  "STRUCT_INFO": "", // struct_info that is either generated or cached

  "EMSCRIPTEN_TRACING": 0, // Add some calls to emscripten tracing APIs

  "USE_GLFW": 2, // Specify the GLFW version that is being linked against.
                 // Only relevant, if you are linking against the GLFW library.
                 // Valid options are 2 for GLFW2 and 3 for GLFW3.

  "WASM": 1, // Whether to use compile code to WebAssembly. Set this to 0 to compile
             // to asm.js.
  ...
  "WASM_BACKEND": 0, // Whether to use the WebAssembly backend that is in development in LLVM.
                     // You should not set this yourself, instead set EMCC_WASM_BACKEND=1 in the
                     // environment.

  "WASM_OBJECT_FILES": 0, // Whether to compile object files as wasm as opposed to the default
                          // of using LLVM IR.

  "BINARYEN_METHOD": "native-wasm", // How we should run WebAssembly code. By default, we run it natively.
                                    // See binaryen's src/js/wasm.js-post.js for more details and options.
  "BINARYEN_SCRIPTS": "", // An optional comma-separated list of script hooks to run after binaryen,
                          // in binaryen's /scripts dir.
  "BINARYEN_IGNORE_IMPLICIT_TRAPS": 0, // Whether to ignore implicit traps when optimizing in binaryen.
  ...
  "BINARYEN_TRAP_MODE": "allow", // How we handle wasm operations that may trap, which includes integer
  ...
  "BINARYEN_PASSES": "", // A comma-separated list of passes to run in the binaryen optimizer,
                         // for example, "dce,precompute,vacuum".
                         // When set, this overrides the default passes we would normally run.
  "WASM_MEM_MAX": -1, // Set the maximum size of memory in the wasm module (in bytes).
  ...
  "BINARYEN_ASYNC_COMPILATION": 1, // Whether to compile the wasm asynchronously, which is more
                                   // efficient and does not block the main thread. This is currently
                                   // required for all but the smallest modules to run in V8
  "BINARYEN_ROOT": "", // Directory where we can find Binaryen. Will be automatically set for you,
                       // but you can set it to override if you are a Binaryen developer.

  "LEGALIZE_JS_FFI": 1, // Whether to legalize the JS FFI interfaces (imports/exports) by wrapping
  ...
  "USE_SDL": 1, // Specify the SDL version that is being linked against.
                // 1, the default, is 1.3, which is implemented in JS
                // 2 is a port of the SDL C code on emscripten-ports
  "USE_SDL_IMAGE": 1, // Specify the SDL_image version that is being linked against. Must match USE_SDL
  "USE_SDL_TTF": 1, // Specify the SDL_ttf version that is being linked against. Must match USE_SDL
  "USE_SDL_NET": 1, // Specify the SDL_net version that is being linked against. Must match USE_SDL
  "USE_ZLIB": 0, // 1 = use zlib from emscripten-ports
  "USE_LIBPNG": 0, // 1 = use libpng from emscripten-ports
  "USE_BULLET": 0, // 1 = use bullet from emscripten-ports
  "USE_VORBIS": 0, // 1 = use vorbis from emscripten-ports
  "USE_OGG": 0, // 1 = use ogg from emscripten-ports
  "USE_FREETYPE": 0, // 1 = use freetype from emscripten-ports
  "USE_HARFBUZZ": 0, // 1 = use harfbuzz from harfbuzz upstream
  "USE_COCOS2D": 0, // 3 = use cocos2d v3 from emscripten-ports

  "SDL2_IMAGE_FORMATS": [], // Formats to support in SDL2_image. Valid values: bmp, gif, lbm, pcx, png, pnm, tga, xcf, xpm, xv
  ...
  "IN_TEST_HARNESS": 0, // If true, the current build is performed for the Emscripten test harness.

  "USE_PTHREADS": 0, // If true, enables support for pthreads.

  "PTHREAD_POOL_SIZE": 0, // Specifies the number of web workers that are preallocated before runtime is initialized. If 0, workers are created on demand.

  "DEFAULT_PTHREAD_STACK_SIZE": 2097152, // If not explicitly specified, this is the stack size to use for newly created pthreads.
  ...
  "PTHREAD_HINT_NUM_CORES": 4,

  "PTHREADS_PROFILING": 0, // True when building with --threadprofiler

  "PTHREADS_DEBUG": 0, // If true, add in debug traces for diagnosing pthreads related issues.

  "MAX_GLOBAL_ALIGN": -1, // received from the backend
  "IMPLEMENTED_FUNCTIONS": [], // received from the backend
  "JSCALL_START_INDEX": 0, // received from the backend
  "JSCALL_SIG_ORDER": {}, // received from the backend
  ...
  "ELIMINATE_DUPLICATE_FUNCTIONS": 0, // disabled by default
  "ELIMINATE_DUPLICATE_FUNCTIONS_PASSES": 5,
  "ELIMINATE_DUPLICATE_FUNCTIONS_DUMP_EQUIVALENT_FUNCTIONS": 0,

  "EVAL_CTORS": 0, // This tries to evaluate global ctors at compile-time, applying their
  ...
  "CYBERDWARF": 0, // see http://kripken.github.io/emscripten-site/docs/debugging/CyberDWARF.html

  "BUNDLED_CD_DEBUG_FILE": "", // Path to the CyberDWARF debug file passed to the compiler

  "TEXTDECODER": 1, // Is enabled, use the JavaScript TextDecoder API for string marshalling.
                    // Enabled by default, set this to 0 to disable.
  "EMBIND_STD_STRING_IS_UTF8": 1, // Embind specific: If enabled, assume UTF-8 encoded data in std::string binding.
                                  // Disable this to support binary data transfer.

  "OFFSCREENCANVAS_SUPPORT": 0, // If set to 1, enables support for transferring canvases to pthreads and creating WebGL contexts in them,
                                // as well as explicit swap control for GL contexts. This needs browser support for the OffscreenCanvas
                                // specification.

  "FETCH_DEBUG": 0, // If nonzero, prints out debugging information in library_fetch.js

  "FETCH": 0, // If nonzero, enables emscripten_fetch API.

  "ASMFS": 0, // If set to 1, uses the multithreaded filesystem that is implemented within the asm.js module, using emscripten_fetch. Implies -s FETCH=1.

  "SINGLE_FILE": 0, // If set to 1, embeds all subresources in the emitted file as base64 string
  ...
  "WASM_TEXT_FILE": "", // For internal use only (name of the file containing wasm text, if relevant).
  "WASM_BINARY_FILE": "", // For internal use only (name of the file containing wasm binary, if relevant).
  "ASMJS_CODE_FILE": "", // For internal use only (name of the file containing asm.js, if relevant).
  "SOURCE_MAP_BASE": "", // Base URL the source mapfile, if relevant

  "MEM_INIT_IN_WASM": 0, // for internal use only

  "SUPPORT_BASE64_EMBEDDING": 0, // If set to 1, src/base64Utils.js will be included in the bundle.
  ...
  "ENVIRONMENT_MAY_BE_WEB_OR_WORKER": 1
}
