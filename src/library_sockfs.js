  ...
  $SOCKFS: {
    ...
    websocket_sock_ops: {
      ...
      handlePeerEvents: function(sock, peer) {
        ...
        var handleOpen = function () {
          ...
          try {
            ...
            while (queued) {
#if SOCKET_DEBUG
              out('websocket sending queued data (' + queued.byteLength + ' bytes): ' + [Array.from(new Uint8Array(queued))]);
#endif
...
#if SOCKET_DEBUG
          out('websocket handle message (' + data.byteLength + ' bytes): ' + [Array.from(data)]);
#endif
      ...
      sendmsg: function(sock, buffer, offset, length, addr, port) {
...
#if SOCKET_DEBUG
            out('websocket queuing (' + length + ' bytes): ' + [Array.from(new Uint8Array(data))]);
#endif
        ...
        try {
#if SOCKET_DEBUG
          out('websocket send (' + length + ' bytes): ' + [Array.from(new Uint8Array(data))]);
#endif
...
#if SOCKET_DEBUG
        out('websocket read (' + bytesRead + ' bytes): ' + [Array.from(res.buffer)]);
#endif
