...
``-gseparate-dwarf[=FILENAME]``
  ...
  ``FILENAME`` can also be ``FILENAME@PATH_IN_FILE`` where ``FILENAME`` is the
  emitted file as before, and ``PATH_IN_FILE`` is the path to the the file
  (which might be a URL to where the file will be hosted, for example).
