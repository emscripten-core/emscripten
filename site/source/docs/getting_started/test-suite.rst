...
By default the test runner will hide all test output and use a single terminal
line to show test results.
...
One you run just one a single test (or one test at at time using ``-j1``) the test
