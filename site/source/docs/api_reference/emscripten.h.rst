...
.. c:function:: void emscripten_push_main_loop_blocker(em_arg_callback_func func, void *arg)
  ...
  .. note::
    - Main loop blockers block the main loop from running, and can be counted to show progress. In contrast, :c:func:`emscripten_async_call` are not counted, do not block the main loop, and can fire at specific time in the future.
