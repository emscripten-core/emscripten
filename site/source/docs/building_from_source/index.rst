...
For using the LLVM wasm backend (recommended), simply build normal upstream LLVM, including clang and wasm-ld (using something like ``-DLLVM_ENABLE_PROJECTS=lld;clang'``), following `that project's instructions <http://llvm.org/docs/CMake.html>`_. The build must not disable the WebAssembly backend (which is included by default).
