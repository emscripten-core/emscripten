...
.. note::
   ...
   - We highly recommended that you *export* functions that are to be called
     ...
     - If you want to include a JS library element (something from a
       ``src/library*.js`` file, for example) so C code can use it, you need to
       add it to ``DEFAULT_LIBRARY_FUNCS_TO_INCLUDE`` to force it to actually be
       included in the build. If you want to export a JS library element, add it
       to ``EXPORTED_FUNCTIONS`` (which also includes the JS symbol, subsuming
