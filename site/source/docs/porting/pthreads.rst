...
Also note that when compiling code that uses pthreads, an additional JavaScript file `NAME.pthread-main.js` is generated alongside the output .js file (where `NAME` is the name of the main file being emitted). That file must be deployed with the rest of the generated code files. By default, `NAME.pthread-main.js` will be loaded relative to the main HTML page URL. If it is desirable to load the file from a different location e.g. in a CDN environment, then one can define the `Module.locateFile(filename)` function in the main HTML `Module` object to return the URL of the target location of the `NAME.pthread-main.js` entry point. If this function is not defined in `Module`, then the default location relative to the main HTML file is used.
